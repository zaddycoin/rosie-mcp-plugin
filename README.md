...
- A remote MCP server pointing at `https://rosie-mcp.vercel.app/api/mcp`
...
The bundled MCP config lives in [`.mcp.json`](./.mcp.json) and points to the hosted Rosie server:

```json
{
  "mcpServers": {
    "rosie": {
      "url": "https://rosie-mcp.vercel.app/api/mcp"
    }
  }
}
```
